...
export const REQUIRED_DIRECTOR_SUITES: ReadonlyArray<DirectorSuite> = [
  ...
  { id: "director-migration-immutability", path: "tests/director-domain-contract.test.ts", npm_script: "test:webgpt:director", ci_step: "ChatGPT Director domain tests", case_name: "migrations 0009 through 0011 upgrade a real 0008 shape and make Director evidence immutable" },
  { id: "director-currency-migration-compatibility", path: "tests/director-domain-contract.test.ts", npm_script: "test:webgpt:director", ci_step: "ChatGPT Director domain tests", case_name: "migrations 0010 and 0011 upgrade an already-ledgered 0009 Grant database without checksum drift" },
  ...
  { id: "director-artifact-import-ui-boundary", path: "src/workbench-ui/App.test.tsx", npm_script: "test:v2:ui", ci_step: "Workbench V2 UI tests", case_name: "records an artifact_import receipt with only an already-registered Artifact identifier" },
  ...
  { id: "director-artifact-import-contract", path: "tests/director-artifact-import-receipt.test.ts", npm_script: "test:webgpt:director", ci_step: "ChatGPT Director domain tests", case_name: "artifact_import proposals reject source locations and enforce SHOT role and MIME semantics" },
  { id: "director-artifact-import-receipt-boundary", path: "tests/director-artifact-import-receipt.test.ts", npm_script: "test:webgpt:director", ci_step: "ChatGPT Director domain tests", case_name: "accepted artifact_import records exactly one immutable, path-free receipt after local byte validation" },
  { id: "director-artifact-import-migration", path: "tests/director-artifact-import-receipt.test.ts", npm_script: "test:webgpt:director", ci_step: "ChatGPT Director domain tests", case_name: "migration 0011 preserves 0010 proposal evidence and adds the immutable import receipt ledger" },
  { id: "director-managed-heartbeat", path: "tests/director-bridge-windows-operations.test.ts", npm_script: "test:webgpt:director", ci_step: "ChatGPT Director domain tests", case_name: "Director Bridge heartbeat schema carries instance, PID and source fingerprints while excluding business-data fields" },
  { id: "director-preactivation-gate", path: "tests/director-bridge-windows-operations.test.ts", npm_script: "test:webgpt:director", ci_step: "ChatGPT Director domain tests", case_name: "Director Bridge real entrypoint remains pre-activation and stops without loading key or database inputs" },
  { id: "director-completion-retry", path: "tests/director-bridge-windows-operations.test.ts", npm_script: "test:webgpt:director", ci_step: "ChatGPT Director domain tests", case_name: "Director Bridge retries one pending completion before polling or re-invoking its handler" },
  { id: "director-completion-dedup", path: "tests/director-bridge-windows-operations.test.ts", npm_script: "test:webgpt:director", ci_step: "ChatGPT Director domain tests", case_name: "Director Bridge broker accepts an identical completion retry but rejects a conflict" },
  { id: "director-windows-lifecycle-boundary", path: "tests/director-bridge-windows-operations.test.ts", npm_script: "test:webgpt:director", ci_step: "ChatGPT Director domain tests", case_name: "Director Bridge Windows lifecycle manager scripts do not read Bridge key or database file contents" },
  { id: "director-windows-runtime-gate", path: "tests/director-bridge-windows-operations.test.ts", npm_script: "test:webgpt:director", ci_step: "ChatGPT Director domain tests", case_name: "Director Bridge fake-runtime smoke command is wired into canonical local and Windows CI scripts" }
];

export const REQUIRED_READONLY_APP_SUITES: ReadonlyArray<ReadonlyAppSuite> = [
  { id: "readonly-projection-ledger-gate", path: "tests/webgpt-cloud-projection.test.ts", npm_script: "test:webgpt:cloud", ci_step: "WebGPT cloud projection tests", case_name: "readonly projection requires migration 0016 and never upgrades an older database" },
...
export const REQUIRED_REMEDIATION_SUITES: ReadonlyArray<RemediationSuite> = [
  ...
  { id: "sr2-worker-outcome-boundary", stage: "SR2", kind: "boundary", path: "tests/workbench-v2-domain.test.ts", npm_script: "test:v2", ci_step: "Workbench V2 domain tests", case_name: "[EEI-TASK-02] provider task persistence failure enters manual reconciliation without losing the paid task ID" },
