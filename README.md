...
本地 Workbench 的“系统 → 只读 App 发布”将上述命令收敛为受 action nonce 和人工确认保护的日常操作面：状态读取不会打开业务行或 DPAPI 私钥，预检不会写 receipt 或远端状态，“预检并发布/续期/恢复”才执行一次签名 Snapshot 替换。状态面按服务端 `ttl_remaining_seconds` 在剩余两小时内提示续期，并对 `no_snapshot`、已过期和远端不可用给出稳定 reason code；一分钟状态刷新只读取公开健康投影，绝不自动发布。浏览器不能传入 profile、数据库路径或远端 URL；Workbench 只使用 `WEBGPT_READONLY_PUBLISHER_PROFILE_PATH` 指定的 Git-ignored profile，未设置时使用 `data/webgpt/publisher/profile.json`。该入口已在 Windows Node `22.23.1`、真实活动库 ledger `0008` 和 Provider 关闭条件下完成真实一键发布、Render restart 后 `no_snapshot → 单次确认发布 → 七工具恢复` 及最终 `db:check`。它仍是人工操作，不会创建计划任务、自动发布或自动续期 Snapshot；Windows 自动启动也保持独立门禁。
