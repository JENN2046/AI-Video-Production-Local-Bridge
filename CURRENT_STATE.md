...
- The Personal Readonly Operations surface provides Human Workbench status, explicit readonly preflight and a confirmed real one-click publish path while keeping automatic synchronization and Windows auto-start disabled. Snapshot v3 has now passed Render restart, `no_snapshot`, one confirmed Human Workbench replacement and seven-tool recovery; the remaining operations gates are automation and multi-user acceptance, not manual recovery correctness.
