...
Readonly Federated OAuth portability 候选路线已推进：PR #34 在 repository runtime 中加入 migration `0008`、不可变 issuer binding、provider-neutral Federated config、严格 `scope`/`scp` 和 current-issuer owner readiness；PR #35 把 discovery/JWKS 收敛到 RFC 8414→OIDC、精确 issuer/JWKS、registration-mode gate 与 DNS-pinned HTTPS transport；PR #36 加入 predefined public-client、JWT/轮换、双用户授权、跨项目与六工具整库零写入门禁。Stage 0 因当前 Auth0 tenant/plan 尚未完成只读 capability 核实而记录 `AUTH0_CAPABILITY_GATE_FAILED`。Stytch 仅有离线 capability fixture；其官方路线还要求应用自行托管公网 authorization/login/consent UI，并核实 standards-compatible issuer，因此未通过外部 gate。活动库仍停留在已验收的 `0007`；上述候选状态不等于 IdP、ChatGPT 外部连接或活动库 cutover 已通过。
