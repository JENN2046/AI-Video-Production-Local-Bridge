...
Baseline: Stabilization Remediation SR0–SR5 delivered sequentially through PR #15–#21; SR6 local release re-acceptance is now the active gate

Candidate version: `0.1.0-beta.3`; MCP service: `webgpt-v4.1.1`; database schema: `workbench-v2-5`

Remediated source baseline: `main@958df57` (`Merge pull request #21`)

Last accepted active-database runtime: Beta 2 closeout at `main@de8c2cd`
...
- Canonical `npm test`: PASS against an isolated disposable database；`npm run test:webgpt:v4`: 46/46；browser smoke: 9/9；Windows runtime smoke 覆盖 graceful stop、forced fallback 和最终 `db:check`
...
Remote CI evidence: PR #9–#13 与 Remediation PR #15–#21 均已合并；这些实现 PR 保持 Windows `Quality and integration` 与 `Browser smoke` 门禁。SR6 候选仍需在独立 PR 中重新通过同一门禁。PR2 establishes the Windows Node 22 CI baseline; every future PR must retain that gate.

## Beta 2 snapshot and SR6 gate

- `0.1.0-beta.2` 的运行实现快照为 `main@fbba4ce`；Closeout 位于 `main@de8c2cd`。在活动数据库完成 SR6 Stage 2 前，Beta 2 仍是最后接受的本地活动库运行基线。
- Remediation 代码已经合入 `main@958df57`，SR6 disposable Stage 1 已通过；候选身份 `0.1.0-beta.3`、`webgpt-v4.1.1` 和 `workbench-v2-5` 仍需通过 SR6 PR 门禁以及单独授权的活动库 Stage 2。
...
- Merge the SR6 candidate only after canonical local tests, Windows Node 22 CI and browser smoke are green.
- Obtain Jenn's separate exact authorization before reading, copying, migrating, writing, or starting the remediated runtime against `data/app.sqlite`.
- Under that authorization, stop Workbench, preserve the migration backup, migrate to `workbench-v2-5`, run `db:check`, verify an isolated restore, execute the read-only golden path and restart recovery, and complete a bounded soak.
