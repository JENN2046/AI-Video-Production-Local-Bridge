...
Readonly Federated OAuth 与 MCP App 单用户路线已经验收：migration `0008`、不可变 issuer binding、provider-neutral Federated config、严格 `scope`/`scp`、Auth0 predefined public-client、签名 Snapshot、Remote MCP Runtime、七个只读工具和 iframe Workbench 已完成真实活动库黄金路径。活动库迁移、`db:check`、业务核心 manifest、隔离恢复、手动发布、关闭—重开和有界 soak 均通过。Personal Readonly Operations 又完成了真实一键 preflight/publish、Snapshot fingerprint 一致性和七工具 owner-only 验收。当前状态为 `JENN_SINGLE_USER_MCP_APP_PASS`、`MANUAL_PUBLISH_OPERATIONAL_READY` 和 `PARTIAL_MULTI_USER_GATE`；第二真实用户已由 Jenn 延期，自动同步与自动启动仍未验收。
...
- Human Workbench 的一键 preflight/publish 已完成真实 owner-only 验收；Render Free 冷启动后的 `no_snapshot → 一键重新发布 → 七工具恢复` 尚未执行，仍需单独授权。
...
- The Personal Readonly Operations surface provides Human Workbench status, explicit readonly preflight and a confirmed real one-click publish path while keeping automatic synchronization and Windows auto-start disabled. Render Free cold-start/Snapshot-loss recovery remains a separate external gate.
