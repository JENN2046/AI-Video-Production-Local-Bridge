...
- Human Workbench 的一键 preflight/publish 已完成真实 owner-only 验收；Snapshot v3 已完成 Render 新进程 `no_snapshot → CLI 单次重新发布 → 七工具恢复`。Human Workbench 一键入口在 v3 上的同一冷启动恢复流程尚未重复执行。
...
- The Personal Readonly Operations surface provides Human Workbench status, explicit readonly preflight and a confirmed real one-click publish path while keeping automatic synchronization and Windows auto-start disabled. Snapshot v3 has passed Render process replacement, `no_snapshot`, CLI republication and seven-tool recovery; repeating that recovery through the Human Workbench one-click action remains a separate operations gate.
