...
Baseline: `0.1.0-beta.4` multi-user readonly code candidate; `0.1.0-beta.3` remains the accepted activity-database runtime

Candidate version: `0.1.0-beta.4`; MCP service: `webgpt-v4.2.0`; database schema: `workbench-v2-5`; migration ledger: `0007`

Candidate source baseline before closeout: `main@d5c644c` (`Merge pull request #28`)

Current accepted active-database runtime: `0.1.0-beta.3` on schema `workbench-v2-5`
...
Remote CI evidence: PR #9–#13、Remediation PR #15–#21、SR6 PR #22、Hotfix PR #23 以及 Descope readonly PR #25–#28 均已合并；这些实现 PR 保持 Windows `Quality and integration` 与 `Browser smoke` 门禁。PR2 establishes the Windows Node 22 CI baseline; every future PR must retain that gate.

## Accepted Beta 3 boundary

- `0.1.0-beta.2` 的运行实现快照为 `main@fbba4ce`，Closeout 位于 `main@de8c2cd`，保留为迁移前回退证据。
- `0.1.0-beta.3`、`webgpt-v4.1.1` 和 `workbench-v2-5` 已通过 SR6 disposable Stage 1 与经授权的 active-database Stage 2，成为 Jenn 当前接受的本地运行基线。
...
- migration `0007` 已通过 fresh/0006-copy/idempotency/schema-drift 测试，但尚未应用到 Jenn 的活动数据库。
- 外部 Descope tenant、ChatGPT connector、Secure MCP Tunnel 和真实多用户黄金路径尚未配置或验收，因此 Beta 4 只是候选代码基线，不替代 Beta 3 的活动库运行验收结论。
- 不创建 Git tag、不发布 package、不执行 release 或 deploy。
