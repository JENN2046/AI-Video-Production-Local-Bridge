...
async function readJsonBody(request: IncomingMessage): Promise<unknown> {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object") throw new Error("INVALID_JSON_RPC");
    return parsed;
  } catch {
    throw new WebGptV4Error("INVALID_JSON_BODY", "MCP request body must contain JSON-RPC data.");
  }
}
...
function normalizedOrigin(value: string): string {
  ...
  if (parsed.origin === "null" || parsed.pathname !== "/" || parsed.search || parsed.hash) {
    throw new WebGptV4Error("DIRECT_OAUTH_CANARY_ORIGIN_INVALID", "Allowed Origins must be absolute origins without a path, query, or fragment.");
  }
  return parsed.origin;
}

function normalizeAllowedOrigins(origins: readonly string[] | undefined): Set<string> {
  return new Set((origins ?? []).map(normalizedOrigin));
}
...
function requireSafeListener(host: string, allowedOrigins: ReadonlySet<string>): void {
  if (!isLoopbackHost(host) && allowedOrigins.size === 0) {
    throw new WebGptV4Error("DIRECT_OAUTH_CANARY_ORIGINS_REQUIRED", "A non-loopback Direct OAuth canary listener requires an explicit Origin allowlist.");
  }
}

function requireAllowedOrigin(request: IncomingMessage, allowedOrigins: ReadonlySet<string>): void {
  const origin = request.headers.origin;
  if (origin === undefined) return;
  if (Array.isArray(origin) || !allowedOrigins.has(origin)) {
    throw new WebGptV4Error("DIRECT_OAUTH_CANARY_ORIGIN_FORBIDDEN", "Request Origin is not allowed for this Direct OAuth canary.");
  }
}
...
export async function startDirectOAuthCanary(options: DirectOAuthCanaryOptions = {}): Promise<DirectOAuthCanaryRuntime> {
  ...
  const handle = async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
    try {
      ...
      sendJson(response, 403, { ok: false, error: safe });
      return;
    }
    ...
    const metadataPath = request.method === "GET" && (
      (url.search === "" && (url.pathname === "/.well-known/oauth-protected-resource" || url.pathname === "/.well-known/oauth-protected-resource/mcp"))
      || (url.pathname === configuredMetadataUrl.pathname && url.search === configuredMetadataUrl.search)
    );
    if (metadataPath) {
    ...
    if (request.method !== "POST") {
      sendJson(response, 405, { ok: false, error: { code: "METHOD_NOT_ALLOWED", message: "This MCP endpoint accepts POST only and does not offer a standalone SSE stream." } }, { allow: "POST" });
      return;
    }
    ...
    try {
      ...
      if (!response.headersSent) sendJson(response, 500, {
        jsonrpc: "2.0", id: safeJsonRpcId(parsedBody), error: { code: -32603, message: "Internal MCP canary error." }
      });
    } finally {
  ...
  return {
    host,
    port,
    mcp_url: `http://${host.includes(":") ? `[${host}]` : host}:${port}/mcp`,
    close: async () => {
      await close(server);
