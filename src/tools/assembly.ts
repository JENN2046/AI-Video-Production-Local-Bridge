...
import {
  ...
  registerAssemblyFinalMediaArtifact,
...
function sqliteFailure(error: unknown): AssemblyFailure {
  ...
  if (mapped.code === "PRODUCTION_MUTATION_CONFLICT") {
    return new AssemblyFailure(mapped.code, mapped.message);
  }
...
export async function queueWorkbenchAssembly(
  ...
  const latestPrior = db.prepare(`SELECT job_id, project_id, job_type, state
    FROM workbench_delivery_jobs
    WHERE project_id = ? AND job_type = 'assembly'
    ORDER BY created_at DESC, rowid DESC LIMIT 1`).get(input.project_id) as {
      job_id: string; project_id: string; job_type: string; state: string;
    } | undefined;
  if (latestPrior && ["failed", "interrupted"].includes(latestPrior.state)
    && input.retry_of_job_id !== latestPrior.job_id) {
    return { ok: false, error: { code: "ASSEMBLY_RETRY_REQUIRED", message: "Explicit retry lineage is required after an interrupted or failed assembly." } };
  }
...
function markAssemblyJobFailed(db: M0Database, jobId: string, code: string, dependencies: AssemblyDependencies): void {
  ...
  try {
    ...
    if (delivery?.workflow_state !== "assembling") {
      throw new AssemblyFailure("ASSEMBLY_INPUT_CHANGED", "Assembly delivery state changed before failure finalization.");
    }
    const terminalEventId = `delivery_event_${uuid(dependencies)}`;
    withWorkbenchProductionMutationAuthority(db, {
      kind: "assembly_failure", project_id: job.project_id, object_id: job.project_id
    }, () => db.prepare(`UPDATE workbench_delivery_state
...
export async function runWorkbenchAssemblyJob(
  ...
  try {
    ...
    let finalizationPhase = "authority_revalidation";
    try {
      connection.exec("BEGIN IMMEDIATE");
      ...
      assertCurrentSnapshot(connection, job.project_id, job.input_fingerprint ?? "");
      finalizationPhase = "artifact_registration";
      const artifact = registerAssemblyFinalMediaArtifact({
        artifact_type: "video",
        role: "final_video",
      ...
      finalizationPhase = "project_reload";
      ...
      finalizationPhase = "generation_run_registration";
      saveGenerationRun(connection, run);
      const timestamp = now(dependencies);
      const terminalEventId = `delivery_event_${uuid(dependencies)}`;
      finalizationPhase = "project_delivery_projection";
      withWorkbenchProductionMutationAuthority(connection, {
        kind: "assembly_finalization", project_id: job.project_id, object_id: job.project_id
      }, () => {
        finalizationPhase = "project_projection";
        const projected = connection.prepare(`UPDATE projects
          SET data_json = ?, updated_at = ? WHERE project_id = ?`)
          .run(JSON.stringify(project), timestamp, job.project_id) as { changes: number };
        if (projected.changes !== 1) {
          throw new AssemblyFailure("PROJECT_NOT_FOUND", "Project was not found during assembly finalization.");
        }
        finalizationPhase = "delivery_projection";
        const delivery = connection.prepare(`UPDATE workbench_delivery_state SET
          workflow_state = 'final_review', current_final_artifact_id = ?, assembly_input_fingerprint = ?,
          approved_artifact_id = NULL, latest_export_id = NULL, last_assembled_at = ?,
          latest_exported_at = NULL, updated_at = ?
          WHERE project_id = ? AND workflow_state = 'assembling'`)
          .run(outputArtifactId, job.input_fingerprint, timestamp, timestamp, job.project_id) as { changes: number };
        if (delivery.changes !== 1) {
          throw new AssemblyFailure("ASSEMBLY_INPUT_CHANGED", "Assembly delivery state changed before finalization.");
        }
      });
      ...
      throw new AssemblyFailure(
        ...
        `Final assembly failed closed during ${finalizationPhase}.`
      );
    }
...
export function interruptUnfinishedWorkbenchDeliveryJobs(
  db = openM0Database(),
  dependencies: AssemblyDependencies = {}
): { interrupted: number; recovery_evidence_preserved: number } {
  ...
  try {
    ...
    for (const job of jobs) {
      ...
      if (job.job_type === "assembly") {
        if (delivery?.workflow_state !== "assembling") {
          throw new AssemblyFailure("ASSEMBLY_INPUT_CHANGED", "Assembly delivery state changed before restart recovery.");
        }
        withWorkbenchProductionMutationAuthority(db, {
          kind: "assembly_interruption", project_id: job.project_id, object_id: job.project_id
        }, () => db.prepare(`UPDATE workbench_delivery_state
            SET workflow_state = 'ready_to_assemble', assembly_input_fingerprint = NULL, updated_at = ?
            WHERE project_id = ?`)
          .run(timestamp, job.project_id));
  ...
  return {
    ...
    recovery_evidence_preserved: [...recoveryEvidence.values()].filter(Boolean).length
  };
}
