...
describe("Human Workbench V2 shell", () => {
  ...
  it("records an artifact_import receipt with only an already-registered Artifact identifier", async () => {
    ...
    expect(fetchMock.mock.calls.some(([input]) => String(input) === `/api/v2/assets/media?scope=all&project_id=${projectId}&status=active&limit=200`)).toBe(true);
