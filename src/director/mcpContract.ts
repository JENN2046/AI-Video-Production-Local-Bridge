...
const descriptions: Record<DirectorNativeToolName, string> = {
  ...
  submit_director_proposal: [
    ...
    "First call get_director_focus and get_director_context. Pass one top-level object with focus_id, focus_generation, base_state_hash, idempotency_key, and proposal. Copy focus.focus_id, map focus.generation to this tool's required focus_generation, and copy base_state_hash from get_director_context. Use a new idempotency_key of 16-160 characters for each distinct proposal. Put kind and payload inside proposal as { kind, payload }; do not place kind or payload at the top level.",
    ...
    "For kind artifact_import, proposal.payload must contain exactly: shot_id, target_role (storyboard_image or generated_clip), expected_mime_type (image/jpeg, image/png, or video/mp4 compatible with target_role), summary, rationale. It only proposes binding an already registered local Artifact to that SHOT; it must not contain an Artifact id, pathname, URL, media bytes, or base64.",
