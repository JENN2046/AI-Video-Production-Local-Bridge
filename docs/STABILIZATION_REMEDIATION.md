...
Target closeout identities:

- Package: `0.1.0-beta.3`
- MCP service: `webgpt-v4.1.1`
- Database schema: `workbench-v2-5`

These identities distinguish the remediated runtime from the Beta 2 snapshot. They do not create a tag, publish a package, release, or deploy anything.
...
- After a schema-changing remediation PR merges, updated `main` binaries are not approved to open `data/app.sqlite` until SR6 receives that authorization. Local production must remain stopped or use the last accepted Beta 2 snapshot at `main@de8c2cd`; syncing source does not authorize starting the new runtime against the active database.
...
2. An Artifact's project/SHOT scope, `artifact_type`, and `role` are immutable after creation. `status` changes only through an explicit lifecycle transition. A project/SHOT reference is created or changed only through one validated, transactional attach operation.
...
- Add a version-controlled test-suite catalog that classifies every `tests/**/*.test.ts` and browser spec as `mandatory` or `historical_non_runtime`. New or renamed test files fail the catalog check until classified.
- `historical_non_runtime` requires an explicit rationale and may be used only when the test no longer exercises an active entrypoint. Active media, Artifact, Provider, credential, generation, database, WebGPT, Workbench, source-audit, and secret-boundary tests are always `mandatory`.
- The initial audit must include currently unselected suites such as `m0-b-media-artifacts`, `m0-g-provider-boundary`, `m0-r1-media-validity`, `m1-0-external-image-transfer`, `m1-provider-boundary`, `provider-env-secret-safety`, memory, source-audit, and legacy integrity. It must not assume that the examples are the complete inventory.
- Add named commands for all cataloged mandatory suites, and require every mandatory suite in both canonical `npm test` and Windows CI. Retain named CI steps or named groups so a failure identifies its boundary.
- Add a meta-test that independently fails when a mandatory suite is omitted from canonical `npm test` **or** from Windows CI; presence on one surface never satisfies the other. It also fails for unclassified files, duplicate catalog entries, missing files, or invalid historical classifications.

Acceptance is a clean Windows run and intentional fixtures proving unclassified tests are rejected and every mandatory suite is selected by both the local canonical command and Windows CI. SR1–SR4 may not start until SR0.5 is merged.
...
- Retain `MediaArtifact` as the production object: one artifact ID, immutable project/SHOT scope, `artifact_type`, `role`, metadata, lifecycle status, and a reference to one blob. `project_id`, `shot_id`, `artifact_type`, and `role` are never updateable after creation; `status` changes only through validated lifecycle transitions.
...
Acceptance includes cross-project checksum reuse, cross-SHOT reuse, rejected role/type mutation, invalid status transitions, concurrent binding attempts, failed binding rollback, and WebGPT/Workbench reads after each case.
...
SR6 also closes the version boundary: package, MCP service, and schema must report the target identities declared above. Until the authorized active-database migration and acceptance pass, the remediated runtime remains a candidate and the Beta 2 snapshot remains the last accepted local runtime.

## Exit gates

External connection work can be proposed only when all of the following are true:

- SR0.5–SR5 are merged with green Windows CI and browser smoke.
- Package, MCP service, and database schema report `0.1.0-beta.3`, `webgpt-v4.1.1`, and `workbench-v2-5` respectively.
