...
1. A media byte sequence is identified by an immutable content checksum; a project or SHOT binding cannot be changed by reusing that content.
2. A project/SHOT artifact reference is created or changed only through one validated, transactional binding operation.
...
Each delivery is one independently reviewable PR based on the latest green `main`. It must pass its scoped tests, `npm run typecheck`, `npm run build`, `npm test`, and `npm run secret:scan` before the next delivery begins.
...
- Retain `MediaArtifact` as the production object: one artifact ID, project/SHOT scope, role, status, metadata, and a reference to one blob.
- Deduplicate only blobs by checksum. Importing identical bytes into another project or SHOT creates a new Artifact; it never rewrites another Artifact's project, SHOT, role, or JSON payload.
- Replace direct reference mutation with a single `bindArtifactToShot`/equivalent domain operation. It validates role, artifact status, project and SHOT scope, then transactionally updates the Artifact binding and the target SHOT reference.
...
- Add migration design, fresh-fixture migration tests, legacy-data classification, and failure-injection tests. No active database migration is run in this PR.
...
- Write incoming media to an app-controlled staging path, calculate SHA-256 and size, run type-specific full validation, then atomically activate the file and its Blob/Artifact records.
...
- Registration failures leave neither an active file without a record nor a record claiming an unverified file.
...
### SR5 — canonical test and CI matrix

Make safety tests mandatory rather than merely available.

- Add `m1-provider-boundary`, memory, and source-audit suites to the canonical `npm test` command and Windows CI.
- Keep named CI steps so failures identify the affected boundary.
- Add the SR1–SR4 fault-injection and migration-copy suites to the appropriate unit/integration lanes.
- Assert that CI uses the canonical test surface or explicitly list every canonical safety suite; prevent future divergence with a meta-test.
