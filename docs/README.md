...
This reconciliation follows canonical source at migration `0014` /
`workbench-v2-9`. That is a source fact, not a new activity-database, runtime,
...
- [Readonly MCP App Delivery](webgpt/READONLY_MCP_APP_DELIVERY_RUNBOOK.md) — historical acceptance and future revalidation boundary; current-main execution requires `0014` admission
- [Unified ChatGPT Workspace Transport](webgpt/UNIFIED_CHATGPT_WORKSPACE_TRANSPORT_RUNBOOK.md) — historical transport/activity evidence and future revalidation boundary; current-main execution requires `0014` admission
