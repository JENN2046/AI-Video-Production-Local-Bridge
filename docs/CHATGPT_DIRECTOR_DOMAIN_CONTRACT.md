...
PR2 freezes the fixed Manual/Native registry and separate Director OAuth resource in [CHATGPT_DIRECTOR_MANUAL_NATIVE_TOOLS.md](CHATGPT_DIRECTOR_MANUAL_NATIVE_TOOLS.md). PR3 implements the isolated authenticated outbound bridge, local authority checks, zero-database-write frame analysis and immutable native Proposal handoff described in [CHATGPT_DIRECTOR_LOCAL_BRIDGE.md](CHATGPT_DIRECTOR_LOCAL_BRIDGE.md). Later PRs must still implement and separately validate the Human Workbench approval queue, bounded RunningHub orchestrator and replaceable memory port. None of those runtime capabilities is claimed as externally operational before its explicit external gate.
