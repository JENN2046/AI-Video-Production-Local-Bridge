...
- 当前 Snapshot/export source、migration 与 runtime authority 一律引用 [Current State](../../CURRENT_STATE.md)；已发布 Snapshot 对 `workbench-v2-10` / `0015`、`workbench-v2-6` / `0011` 的历史 previous-source compatibility 仅用于验证和继续读取，不授权新的发布或 runtime transition
...
- Auth0 Stage 0 标准能力探针已通过；External Stage 3 又完成专用 API、predefined public client、精确 ChatGPT redirect、真实 token audience/scope、Jenn issuer-bound owner、七工具与 Workbench 验收。`JENN_SINGLE_USER_MCP_APP_PASS` 和 `MANUAL_PUBLISH_OPERATIONAL_READY` 是 ledger `0008`/`0010` 的历史 owner-only 证据；ledger `0011` 仅表示该历史 Auth0 验收当时的 commit-scoped Activity Runtime boundary，证据不得自动转移到后来版本。当前 Activity Runtime authority 一律引用 [Current State](../../CURRENT_STATE.md)，本 runbook 不再复制当前 migration/runtime status；Unified 运行时发布/恢复仍遵循自己的有界人工确认。
...
- 历史 Descope 验收发生在 migration `0010`；其 principal、issuer binding、membership 与 append-only events 保留，但不参与当前 Auth0 issuer readiness。Ledger `0011` 仅表示该历史 Descope 验收当时的 commit-scoped Activity Runtime boundary；证据不得自动转移到后来版本，也不得据此重复迁移或回退。当前 Activity Runtime authority 一律引用 [Current State](../../CURRENT_STATE.md)，本 runbook 不再复制当前 migration/runtime status。
