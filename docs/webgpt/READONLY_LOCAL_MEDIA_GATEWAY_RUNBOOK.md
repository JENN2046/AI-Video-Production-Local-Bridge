...
Before any SQLite connection opens, the matrix establishes descriptor leases for every directory from the database volume root through the fixture run directory, plus the fixture `app.sqlite` and its `-wal` and `-shm` sidecars. Each directory lease requires the opened descriptor and current path to identify the same directory; each file lease additionally requires a single-link regular file inside the fixture root. Alongside the guarded database path, the matrix sends only fixed-width directory and file identities to the Windows path guard over the helper's private stdin. The guard opens the ancestor directories from root to leaf with no-follow directory handles, read/write sharing but no delete sharing, compares each handle with its expected lease identity, then atomically opens and identity-checks the three existing database files under the locked chain before reporting `LOCKED`. It continuously rechecks both the ancestor and file handles while held. SQLite opens and all queries or mutations occur only while both layers remain valid. An ancestor rename or junction ABA swap, temporary file replacement, pre-positioned external sidecar, hard link, or reparse point therefore fails as `MEDIA_ACCEPTANCE_ROOT_UNSAFE` instead of binding SQLite to an unleased file. Both the read-only Snapshot exporter and writable revocation connection retain post-open lease checks as defense in depth.
