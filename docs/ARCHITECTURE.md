...
flowchart LR
    ...
    UW -. "encrypted 5-minute capability; fixture MP4 playback PASS" .-> CF["Cloudflare media ingress — candidate"]
...
- `Readonly Media Gateway`: local `127.0.0.1:2092`, candidate external media path with one accepted isolated Unified MP4 fixture playback path; byte-range remains pending.
...
- `media.skmt617.top` serves the Cloudflare media route. An isolated Unified fixture passed instance-bound health and MP4 playback; byte-range, broader recovery, revocation and persistence acceptance remain separate.
