...
The publish surface is `PUT /snapshot`. A publisher sends a strict `readonly-snapshot-envelope-v2` containing a `readonly-snapshot-v2` finalized Snapshot, key id, Ed25519 signature and no credentials. The v2 contract requires the shared operational state on every public SHOT. The signature covers a v2 domain-separated JCS representation of the complete finalized Snapshot. A replacement is accepted only after envelope version, Snapshot version, schema, fingerprint, TTL, key id and signature verification; v1 payloads fail with a stable unsupported-version error, an older Snapshot cannot replace a newer one, and an identical signed Snapshot is idempotent.
