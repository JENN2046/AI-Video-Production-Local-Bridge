...
The publish surface is `PUT /snapshot`. A publisher sends a strict `readonly-snapshot-envelope-v3` containing a `readonly-snapshot-v3` finalized Snapshot, key id, Ed25519 signature and no credentials. The v3 contract requires shared operational state on every public SHOT and preserves canonical `reason_codes` on overview blockers. The signature covers a v3 domain-separated JCS representation of the complete finalized Snapshot. A replacement is accepted only after envelope version, Snapshot version, schema, fingerprint, TTL, key id and signature verification; earlier payloads fail with a stable unsupported-version error, an older Snapshot cannot replace a newer one, and an identical signed Snapshot is idempotent.
