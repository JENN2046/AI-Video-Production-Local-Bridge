...
test("migration 0008 creates immutable issuer bindings and preserves append-only authorization events", () => {
  ...
  try {
    ...
    assert.equal(result.applied.at(-1), "0016");
...
test("an existing 0007 database applies 0008 through 0011 without guessing issuer bindings", () => {
  const db = new DatabaseSync(":memory:");
  try {
    ...
    assert.deepEqual(runDatabaseMigrations(db).applied, ["0008", "0009", "0010", "0011", "0012", "0013", "0014", "0015", "0016"]);
    assertSchemaCurrent(db);
