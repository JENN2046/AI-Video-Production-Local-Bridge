...
const TASK = "R3-8J_RUNNINGHUB_REAL_KEYFRAME_SINGLE_SUBMIT_CANARY";
const PREP_REPORT_PATH = "data/reports/r3_8i_runninghub_real_keyframe_authorization_prep_result.json";
const G0_FREEZE_REPORT_PATH = "data/reports/g0_r1_package_freeze_result.json";
const OUTPUT_REPORT_PATH = "data/reports/r3_8j_runninghub_real_keyframe_single_submit_canary_result.json";
const OUTPUT_DIR_RELATIVE = "data/media/provider-canary/r3-8j-runninghub-real-keyframe/";
...
const AUTHORIZATION_SHA256 = "678bdd62547bc029b8e8f9ff67684a05271bb57b3c08caac38277eda6668ab8f";
...
type R3_8JResult = "PASS_LIVE_SINGLE_SUBMIT_COMPLETED" | "PROVIDER_FAILED" | "BLOCK_WITH_REASON";
...
async function postJson(input: {
  ...
  if (!response.ok) {
    return { ok: false, error: mapRunningHubProviderError({ http_status: response.status, payload, secrets: [input.credential] }) };
  }
...
async function uploadRunningHubMedia(input: {
  ...
  let host = "";
  try {
    host = new URL(parsed.download_url).host;
  } catch {
    return {
      ok: false,
      error: {
  ...
  return { ok: true, download_url: parsed.download_url, download_url_host: host };
}
...
function baseReport(input: {
  ...
  return {
    task: TASK,
    result: "BLOCK_WITH_REASON" satisfies R3_8JResult,
    mode: process.argv.includes("--live") ? "live" : "dry_run",
    generated_at: new Date().toISOString(),
    source_reports: {
      r3_8i_authorization_prep: PREP_REPORT_PATH,
      g0_freeze: G0_FREEZE_REPORT_PATH
    },
    authorization: {
      required_for_real_call: true,
      provided: Boolean(process.env.R3_8J_AUTHORIZATION_SHA256),
      accepted: process.env.R3_8J_AUTHORIZATION_SHA256 === AUTHORIZATION_SHA256,
      mechanism: "R3_8J_AUTHORIZATION_SHA256",
...
async function main(): Promise<JsonRecord> {
  ...
  try {
    const prepReport = readJson<JsonRecord>(PREP_REPORT_PATH);
    ...
    const report = baseReport({ envLoad, prepReport, freezeReport, artifact, sourceReadable, storageValidation });
    const credential = process.env.RUNNINGHUB_API_KEY ?? "";
    const secrets = [credential].filter(Boolean);

    if (!process.argv.includes("--live")) return block(report, "R3-8J live runner requires --live.");
    if (process.env.R3_8J_AUTHORIZATION_SHA256 !== AUTHORIZATION_SHA256) return block(report, "R3-8J live call requires the matching current authorization hash.");
    if (!envLoad.credential_present || !credential) return block(report, "RUNNINGHUB_API_KEY is not present.");
    if (prepReport?.result !== "PASS_READY_FOR_USER_AUTHORIZATION") return block(report, "R3-8I authorization prep report is missing or not PASS.");
    if (prepReport?.exact_authorization_phrase && typeof prepReport.exact_authorization_phrase === "string" && !prepReport.exact_authorization_phrase.includes("duration_seconds=3")) {
      return block(report, "R3-8I authorization phrase does not contain duration_seconds=3.");
    }
    const prepProjectLinkage = payloadObject(prepReport?.project_linkage);
    if (prepProjectLinkage.runninghub_canary_duration_seconds !== DURATION_SECONDS) return block(report, "R3-8I duration does not match 3 seconds.");
    ...
    if (artifact.artifact_type !== "image" || artifact.role !== "storyboard_image" || artifact.status !== "active") {
      return block(report, "Selected artifact is not an active storyboard_image image.");
    }
    ...
    report.live_execution = {
      ...(report.live_execution as JsonRecord),
      upload_call_count: 1,
      network_call_attempted: true,
      runninghub_called: true
    };
    report.provider_boundary = {
      ...(report.provider_boundary as JsonRecord),
      network_call_attempted: true,
      runninghub_called: true,
    ...
    report.live_execution = {
      ...(report.live_execution as JsonRecord),
      query_call_count: polled.attempts
    };
    report.provider_boundary = {
      ...(report.provider_boundary as JsonRecord),
      status_poll_attempted: polled.attempts > 0
    };
    if (!polled.ok) {
      report.result = "PROVIDER_FAILED";
    ...
    if (polled.status !== "succeeded") {
      ...
      (report.live_execution as JsonRecord).error = safeError(
        polled.mapped_error ?? {
          code: "PROVIDER_REQUEST_FAILED",
          message: `RunningHub task ended with status ${polled.provider_status}.`,
          retryable: false
        },
        secrets
      );
      ...
      (report.live_execution as JsonRecord).error = safeError(
        {
          ...
          message: "RunningHub task succeeded without an output URL.",
          retryable: false
        },
        secrets
      );
    ...
    report.live_execution = {
      ...(report.live_execution as JsonRecord),
      output_download_attempted: true
    };
    report.provider_boundary = {
      ...(report.provider_boundary as JsonRecord),
      output_download_attempted: true
...
if (containsForbiddenLiveLeak(report, [credential])) {
  report.result = "BLOCK_WITH_REASON";
  report.blocked_reason = "R3-8J report contained forbidden secret-shaped, signed URL, base64, or raw provider payload text.";
  report.provider_boundary = {
    ...(report.provider_boundary as JsonRecord),
    secret_values_exposed: false,
    raw_provider_payload_recorded: false
